...
function startGetUsers(){
    getServerData("http://localhost:3001/users").then(
...
function delRow(btn) {
    ...
    fetch(`http://localhost:3001/users/${id}`, fetchOptions).then(
        resp => resp.json(),
        err => console.error(err)
    ).then(
        data => {
...
function createUser(btn) {
    ...
    fetch(`http://localhost:3001/users`, fetchOptions).then  (
        resp => resp.json(),
        err => console.error(err)
    ).then(
        data => startGetUsers()
...
function setRow(btn) {
    ...
    fetch(`http://localhost:3001/users/${data.id}`, fetchOptions).then (
        resp => resp.json(),
        err => console.error(err)
    ).then (
        data => startGetUsers()
